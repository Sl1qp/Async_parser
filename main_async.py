...
async def download_xls(url: str) -> str:
    ...
    print(f"Файл {file_path[12:]} сохранён!")
    return file_path
...
async def parse_table(table_name, async_engine, AsyncSession):
    ...
    await create_and_save_data(async_engine, AsyncSession, filtered_td, trade_date)


async def create_and_save_data(async_engine, AsyncSession, filtered_td, trade_date):
    async with AsyncSession(bind=async_engine) as session:
        for index, row in filtered_td.iterrows():
            ...
            await db.async_insert_to_db(trade_obj, session)


async def main():
    async_engine, AsyncSession = db.get_engine_and_session()
    await db.create_table(async_engine)
    ...
    while True:
        ...
        tasks_for_parse = [
            asyncio.create_task(parse_table(table_name, async_engine, AsyncSession))
